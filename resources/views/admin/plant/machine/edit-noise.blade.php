        ...
        <form action="{{route('admin.plant-machine-update-noise',[
            ...
            <div class="form-group mb-4">
                ...
                <select class="form-select" aria-label="Default select example">
                    ...
                    <option value="{{$symton_noise['symton_name']}}" id="symton-noise-input" for="symton_noise" {{ $data['machineProblem']['symton_noise'] == $symton_noise['symton_name'] ? "selected" : "" }}>{{$symton_noise['symton_name']}}</option>
                    @endforeach
            ...
            <div class="form-group mb-4">
                ...
                <select class="form-select" aria-label="Default select example" name="area" required value="{{$data['machineProblem']['area']}}">
                    ...
                    <option value="{{$area['area']}}" id="causing-part-input" {{ $data['machineProblem']['area'] == $area['area'] ? "selected" : "" }}>{{$area['area']}}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
            ...
            <div class="form-group mb-4">
                ...
                <select class="form-select" aria-label="Default select example">
                    <option>Open this select menu</option>
                    @foreach($data['methods'] as $method)
                    <option value="{{$method['method']}}" id="method-input" {{ $data['machineProblem']['method'] == $method['method'] ? "selected" : "" }}>{{$method['method']}}</option>
                    @endforeach
            ...
            <div class="form-group mb-4 {{$data['machineProblem']['method'] != 'Idle' ? 'd-block' : 'd-none'}}" id="at-gear-wrap">
                ...
                <select class="form-select" aria-label="Default select example">
                    ...
                    <option value="{{$at_gear['gear']}}" id="at-gear-input" {{ $data['machineProblem']['at_gear'] == $at_gear['gear'] ? "selected" : "" }}>{{$at_gear['gear']}}</option>
                    @endforeach
                </select>
            ...
            <div class="justify-content-center align-items-center row mx-0 mb-5">
                <div class="col-12 px-0 px-lg-2 text-center col-lg-5 rounded p-4 align-items-center justify-content-center">
                    <video src="{{ asset('storage/machine_vidio/'.$data['machineProblem']['vidio']) }}" controls class="w-100 vidio-container">
                        Your browser does not support HTML video.
                    </video>
                    <label type="button" class="my-4 btn bg-base text-white rounded w-50 h-25" for="file" accept="image/*">Upload Vidio Engine</label>
